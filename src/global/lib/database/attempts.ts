...
export async function createQuizAttempt(quizId: string, userId: string): Promise<string> {
...
export async function submitQuizAttempt(
  ...
  const questions = quiz.questions.sort((a: any, b: any) => a.question_order - b.question_order)
...
export async function getAttemptWithResults(attemptId: string): Promise<AttemptWithDetails | null> {
  const supabase = createClient()
...
function checkAnswer(question: any, userAnswer: any): boolean {
  ...
  switch (question_type) {
    case 'single_choice':
      const correctOption = question_data.options.find((opt: any) => opt.is_correct)
      return userAnswer === correctOption?.id

    case 'multiple_choice':
      const correctOptions = question_data.options.filter((opt: any) => opt.is_correct).map((opt: any) => opt.id)
    ...
    case 'sequence':
      ...
      const userSequence = Array.isArray(userAnswer) ? userAnswer.map((item: any) => item.id) : []

      return (
        correctSequence.length === userSequence.length &&
        correctSequence.every((id: string, index: number) => userSequence[index] === id)
      )
    ...
    case 'drag_word':
      if (question_data.sentence_template && question_data.blanks) {
        ...
        return question_data.blanks.every((blank: any) => {
          const userBlankAnswer = answers.find((ans: any) => ans.blank_id === blank.id)
          return userBlankAnswer && blank.correct_answers.includes(userBlankAnswer.word_text)
        })
        ...
        return question_data.drag_zones.every((zone: any) => {
          ...
          return (
            ...
            zone.correct_items.every((itemId: string) => zoneItems.includes(itemId))
          )
        })
      }
      return false
